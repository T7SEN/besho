...
| Add Telegram / WhatsApp / Signal / any third-party messenger as a notification fallback | Banned — FCM is the sole sanctioned push transport (`AGENTS.md` § 3.2 + `references/push-routing.md`). Out-of-band messenger fallback was considered and explicitly rejected; reintroducing duplicates the channel and adds an unlocked third-party dependency | None — push delivery degrades to the durable `notifications:{author}` history record per the existing FCM-defensive design (§ 3.3); harden FCM registration if reliability is the concern |
...
| Modify `decrypt()` to skip the session-epoch check                       | Bypass would silently break the Sessions section on `/admin/devices` force-logout | Keep the read; if perf matters extend the 5s in-process cache               |
