...
| `/admin/push-test`          | Send custom FCM + tap-to-fill presets                                                         | `sendTestPushAction` (form-bound via `useActionState`)                                                    |
...
| `/admin/health`             | Tabbed: Health (Redis/FCM/cron/time-integrity/repair) · Cooldowns · Time (snapshot + converter) | `getHealthSnapshot`, `getCronTelemetry`, `getCooldownState`, `repairIndexes`, `repairObedienceDrift`, `migrateObedienceBucketShift` |
