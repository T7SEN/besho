...
| Key                          | Type   | TTL                                       | Description                                                                                                                                                                                |
| ---------------------------- | ------ | ----------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `tod:challenge:{id}`         | JSON   | none (until soft-delete)                  | `{ id, issuer, recipient, truthPrompt, darePrompt, pick, status, response, createdAt, pickedAt, respondedAt, closedAt, expiresAt, refuseReason?, adminCancelReason?, withdrawReason? }`. The three reason fields are non-overlapping by origin: `refuseReason` written by `refuseChallenge` (recipient), `adminCancelReason` written by `forceCancelTodChallenge` / `/api/admin/cli/tod/cancel` (Sir admin), `withdrawReason` written by `withdrawChallenge` (issuer own-cancel).    |
...
| `tod:active:T7SEN`           | STRING | `TOD_PENDING_TTL_SEC` (72h) → `TOD_PICKED_TTL_SEC` (48h) on pick | Single-slot sentinel for Sir's outgoing direction. `SET` on `issueChallenge`, refreshed (tighter TTL) on `pickPrompt`, `DEL` on terminal transition.                                       |
...
| `tod:stats:{author}`         | HASH   | none                                      | Per-author counters: `issued` / `truthsAnswered` / `daresCompleted` / `refused` / `safeworded` / `expired` / `withdrawn` / `currentStreak` / `longestStreak`. Both authors' stats are visible to both — same transparency as `/review` and `/rewards`. Streak fields are recipient-side: `currentStreak` increments on `submitResponse` and resets on `refuseChallenge` + cron-driven expiry; safeword + withdrawn do NOT reset (free outs by design). `longestStreak` is a high-water mark — only ever increases via the normal flow, but admin stat edits can correct it. |
| `tod:prompts:{author}`       | JSON   | none                                      | Per-author private prompt library — array of `TodPrompt = { id, truthPrompt, darePrompt, label?, createdAt }`. Capped at `MAX_PROMPTS_PER_LIBRARY` (50). Each author manages their own; there is NO cross-author view (Sir cannot see Kitten's library and vice versa). The issue form pulls from the caller's own library via the Library Sheet. |
| `tod:fcm:expire-warn:{id}`   | STRING | `expiresAt - now` (per record)            | Dedup sentinel for the cron-driven "expires soon" push. `SET NX EX` — first acquisition wins; TTL bounded by the challenge's remaining lifetime so the key auto-evicts on natural expiry (no orphan sentinels surviving past terminal transitions). |
| `reactions:tod:{id}`         | HASH   | none                                      | Per-challenge reactions, field = author, value = emoji. Same shape as `reactions:{noteId}` for notes; new target types use the prefixed form. Only rendered on `status === "completed"` rows. Auxiliary state — not preserved on soft-delete restore. |

`status` is `'pending'` | `'picked'` | `'completed'` | `'refused'` | `'safeworded'` | `'expired'` | `'withdrawn'` | `'cancelled'`. `'pending'` and `'picked'` are active; everything else is terminal. Writes via `issueChallenge` / `pickPrompt` / `submitResponse` / `refuseChallenge` / `safewordChallenge` / `withdrawChallenge` (user-facing) and `forceCancelTodChallenge` / `resetTodStats` / `adjustTodStat` / `purgeAllTodChallenges` (Sir-only admin). `expireDueChallenges` is **cron-only** — page reads MUST NOT mutate state. TTL defaults: `TOD_PENDING_TTL_SEC = 72h`, `TOD_PICKED_TTL_SEC = 48h`. The pending window dropped from 7d to 72h in the v2 retune — the conversational half-life of a dare goes stale beyond ~3 days, so the cron now sweeps stuck challenges faster.
...
`pickPrompt(id, "truth" | "dare")` locks the recipient into one prompt type and resets the expiry to `pickedAt + TOD_PICKED_TTL_SEC`. The other prompt is discarded conceptually but stays on the record (rendered in admin history). `submitResponse(id, formData)` requires `status === "picked"` and a non-empty response ≤ `MAX_RESPONSE_LEN` (2000 chars); writes the response, flips to `completed`, increments the recipient's `truthsAnswered` or `daresCompleted` (chosen by `pick`), DEL's the issuer's active sentinel, bumps `currentStreak` (with longest-streak high-water update), and emits the obedience event (Kitten only). No optimistic UI on `submitResponse` — the response field is the load-bearing artifact and a rollback would leave it visually present but server-absent.

**Change-of-mind window:** `pickPrompt` accepts a `picked → picked` flip to the other type when `now - pickedAt < CHANGE_PICK_WINDOW_SEC * 1000` (60s default). The new pick replaces the old, `pickedAt` advances to the flip time (so the window resets per change). Past the window, the standard "no longer pending" error returns. The issuer notification is suppressed on the flip — double-pinging within a minute is noise. Client-side, `<IncomingCard>` resets the response draft state when `challenge.pick` changes so a half-typed answer doesn't carry across pick types.
...
`data: { url: "/games/truth-or-dare", kind: "tod_challenge", challengeId, title, body }`. The `kind` field is a routing discriminant. The foreground listener in `<FCMProvider>` branches on `kind === "tod_challenge"` and dispatches `TOD_ARRIVED_EVENT` (`"ourspace:tod-arrived"`). The `/games/truth-or-dare` page subscribes via `addEventListener` and re-fetches its bundle on receipt — surfacing the new challenge immediately rather than waiting for the next pull-to-refresh. When the recipient is on a different page, the FCM-provider branch ALSO dispatches the standard PushToast so they see the arrival; when they're already on the TOD page, only the dispatch fires (no duplicate toast). Background notifications carry the standard heads-up banner with `data.url` pointing at `/games/truth-or-dare`. Presence-aware: if the recipient is on `/games/truth-or-dare` AND server-side presence is fresh, `sendNotification` skips the push entirely and the page's existing refresh handles the update.

### Expiration sweep

`/api/cron/timer-expire` calls `expireDueChallenges(200)` alongside `expireDueDirectives` and `expireDuePunishments`. Walks the most-recent 200 challenges, transitions any active record past `expiresAt` to `expired`, increments `tod:stats:{recipient}.expired`, `HDEL`s `currentStreak` (expiry resets the streak), emits `tod_expired` (Kitten only), and fires a both-author FCM (the expiry is bilateral information — both lost something). Soft-fail per record — per-challenge errors log but don't abort the sweep.

### Pre-warning sweep

`warnExpiringChallenges(200)` runs on the same `/api/cron/timer-expire` tick AFTER `expireDueChallenges` (so a record that just expired this tick isn't double-touched). Walks active records where `expiresAt - now < TOD_EXPIRE_WARN_THRESHOLD_SEC` (24h default), acquires `tod:fcm:expire-warn:{id}` via `SET NX EX` with TTL bounded by the remaining lifetime, and fires a one-shot push to the recipient — "⏳ TOD challenge expires soon · ~Nh left to {pick|answer}". The sentinel auto-evicts on natural challenge expiry so the storage cost is bounded. Fires for both `pending` (24h to pick) and `picked` (24h to answer) phases. Best-effort: per-record FCM failures log but don't abort the sweep.

### Prompt library

`tod:prompts:{author}` is a JSON array of `TodPrompt` records — each author's private library of pre-authored truth+dare pairs. Capped at `MAX_PROMPTS_PER_LIBRARY = 50`; the optional `label` is capped at `MAX_PROMPT_LABEL_LEN = 60` chars. Actions:

- `getTodPromptLibrary()` — reads the caller's own library (no cross-author access by design; the action keys off `getSession().author`).
- `saveTodPrompt({ truthPrompt, darePrompt, label? })` — appends a new entry; refuses past the cap. Restraint-gated for Kitten.
- `deleteTodPrompt(promptId)` — removes one entry; idempotent. Restraint-gated for Kitten.

UI: `<IssueForm>` has a Sheet-triggered Library picker (bottom sheet on mobile) and a "Save to library" button next to the Issue submit. The Sheet supports per-row delete with hover-revealed affordance on md+.

### Reactions on completed challenges

`reactions:tod:{challengeId}` HASH with field = author, value = emoji. Uses the generalized reactions module (`reactToTarget("tod", id, emoji)` / `getReactionsForTargets("tod", ids)`). Bundled into `getTodBundle.reactions` and `getChallengeHistory.reactions` so the page renders without a separate fetch. UI gate: `<TodReactions>` only renders on `status === "completed"` rows — terminal-pre-pick states have no content worth reacting to. Auxiliary state — intentionally NOT preserved on soft-delete restore (mirrors the notes pattern in `references/coding-patterns.md` § "Soft-Delete via Trash Helper").

### Windowed admin stats

`getTodWindowedStats(windowDays?)` is a Sir-only aggregation that walks `tod:challenges:index` filtered by `createdAt` score range (or all-time when omitted). Returns per-author `TodWindowedCounters` — `issued` counts on the issuer side, `truthsAnswered`/`daresCompleted`/`refused`/`safeworded`/`expired` on the recipient side, `withdrawn` on the issuer side. Cancelled records intentionally don't count anywhere (admin overrides are clean rollbacks). The Stats tab on `/admin/games/truth-or-dare` renders an All/30d/7d toggle above the existing editable cumulative counters; the cumulative HASH at `tod:stats:{author}` is the editable source-of-truth, the windowed view is a read-only computation from the index.
...
| `rewards:claim-reopen:{author}:{weekKey}`    | INT    | none | Sir-set reopen flag. When present, `claimReward` accepts that week regardless of the normal window. DEL'd on successful claim — one-shot. Value is the reopen timestamp (string); only truthiness consulted. Set by `adminReopenClaimWindow`. |
| `rewards:claim-reopen:index:{author}`        | ZSET   | none | Index of currently-reopened weeks per author. Score = reopen ts, member = weekKey. ZREM'd alongside the flag on successful claim. Lets `getRewardsBundle` enumerate active reopens without SCAN. |
| `obedience:streak:{author}`                  | INT    | none | Consecutive prior high-score weeks (entering current week). Sir-tunable via `adminSetStreakRaw`. Live recompute drives the CURRENT week's display multiplier. |
| `obedience:entry-streak:{author}:{weekKey}`  | INT    | none | Snapshot of the streak value ENTERING this week. Written by the prior week's `finalizeWeek`. Used by `finalizeWeek` of THIS week (and by `computeWeekScore` for past unfinalized weeks) so streak adjustments in the gap between week-end and cron-run don't retroactively shift the past week's display or poison the next finalize's frozen multiplier. `adminSetStreakRaw` also writes this for the current weekKey so Sir's overrides carry into the next finalize. |
...
| `obedience:week-wrapped-fcm:{author}:{weekKey}` | STRING | 60d  | Sentinel — set NX once the "📊 Week wrapped" recap FCM has fired. Independent of `finalizedKey` so the recap stays once-per-week even if the finalize lock is re-acquired. Value is the firing timestamp (debug aid). Set inside `notifyWeekClosed`. |
...
| `obedience:manual-reasons:{author}:{weekKey}` | HASH   | none | Per-week manual_adjust reasons. Field = `eventId`, value = reason text. Joined into the audit/events read by `getEventLog` and `computeWeekScore` so the Event log row + Breakdown row can render the reason instead of the generic "Manual adjustment" label. ONLY for `manual_adjust` — other event types aggregate by type and don't carry per-event prose. |
...
- `src/lib/games/truth-or-dare-constants.ts` — `ChallengeStatus` / `ChallengeType` / `TodChallenge` / `TodStats` / `TodPrompt` types, `ACTIVE_STATUSES` / `TERMINAL_STATUSES` literals, `MAX_PROMPT_LEN` / `MAX_RESPONSE_LEN` / `MAX_REFUSE_REASON_LEN` / `MAX_CANCEL_REASON_LEN` / `MAX_PROMPT_LABEL_LEN` / `MAX_PROMPTS_PER_LIBRARY` bounds, `TOD_PENDING_TTL_SEC` (72h) / `TOD_PICKED_TTL_SEC` (48h) / `CHANGE_PICK_WINDOW_SEC` (60s) / `TOD_EXPIRE_WARN_THRESHOLD_SEC` (24h) / `TOD_HISTORY_PAGE_SIZE`, `TOD_PAYLOAD_KIND`, `TOD_ARRIVED_EVENT`, Redis key helpers (`TOD_INDEX_KEY`, `todChallengeKey`, `todActiveKey`, `todStatsKey`, `todPromptsKey`, `todExpireWarnSentinelKey`), `STATUS_LABELS` / `STATUS_CHIP` / `TOD_STAT_LABELS` display maps
- `src/lib/games/truth-or-dare-utils.ts` — `formatRelative`, `formatCountdown` (pure rendering helpers shared by the user page + admin page; no Redis, no async)
- `src/app/actions/games/truth-or-dare.ts` — user-facing actions including `getTodPromptLibrary`, `saveTodPrompt`, `deleteTodPrompt` (library), `warnExpiringChallenges` (cron walker)
- `src/app/actions/admin/games.ts` — admin actions including `getTodWindowedStats` (`TodWindowedCounters`, `TodWindowedStats`)
- `src/app/actions/reactions.ts` — generalized `reactToTarget` / `getReactionsForTargets` + `ReactionTargetType`, legacy `reactToNote` / `getReactionsForNotes` wrappers, `reactionKeyFor(type, id)` storage-asymmetry helper
- `src/components/reactions/reaction-strip.tsx` — generic reaction UI (variant default/compact)
- `src/components/notes/note-reactions.tsx` — thin notes wrapper
- `src/components/games/truth-or-dare/` — `page-header.tsx`, `incoming-card.tsx` (+ inline `PromptCard`), `outgoing-card.tsx`, `issue-form.tsx` (now Sheet-based prompt library picker + Save to library), `history-row.tsx`, `tod-reactions.tsx` (thin wrapper around `<ReactionStrip variant="compact">`), `stats-strip.tsx` (+ inline `StatsColumn`)
- `src/components/admin/games/truth-or-dare/` — `page-header.tsx`, `tab-button.tsx` (+ `AdminTodTab` type), `active-tab.tsx` (+ inline `MassCancelButton`, `ActiveRecordCard`), `history-tab.tsx` (+ inline `AdminHistoryRow`), `stats-tab.tsx` (+ inline `WindowedColumn`, `StatsEditor`; All/30d/7d windowed view)
