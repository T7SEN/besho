...
│   ├── admin/                  # Sir-only sub-tree (layout redirects non-Sir): trash, export, push-test, activity, sessions, devices (presence + FCM tokens + per-install records — old /admin/inspector merged in), stats, health (tabbed: Health / Cooldowns / Time — old /admin/cooldowns, /admin/time, and /admin/timezone merged in; the Time tab hosts both the live snapshot and the Cairo↔Tabuk converter), auth-log, mood, dates, rewards, permissions, notifications, restraint-history, redis
