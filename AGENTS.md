...
│   ├── admin/                  # Sir-only sub-tree (layout redirects non-Sir): trash, export, push-test, devices (presence + FCM tokens + force-logout sessions + per-install records — old /admin/inspector and /admin/sessions merged in), stats, health (tabbed: Health / Cooldowns / Time — old /admin/cooldowns, /admin/time, /admin/timezone merged in; Time tab hosts live snapshot + Cairo↔Tabuk converter), logs (tabbed: Activity / Outbound / Restraint / Auth failures — old /admin/activity, /admin/notifications, /admin/restraint-history, /admin/auth-log merged in), mood, dates, rewards, permissions, redis
