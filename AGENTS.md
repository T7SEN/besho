...
Algorithm: (1) `pushNotificationToHistory(target, payload)` first, (2) read `presence:{author}` (TTL 15s), (3) if recipient is on the target page → skip push (SSE handles UI), (4) else FCM — foreground (presence exists, different page) gets a **data-only** payload (in-app `PushToast`); background/closed gets a full `notification` payload (OS heads-up banner). The `notification` field MUST NOT be set in the foreground payload, or Android double-notifies. **No Web Push fallback.** Full algorithm and failure modes: `references/push-routing.md`.
...
- **Ritual window-open FCM is cron-driven from outside Vercel.** `/api/cron/ritual-windows` calls `getRitualsForCron(LOOKBACK_MS)` — a lean read (1 ZRANGE + 1 MGET, plus an occurrence HGETALL only for rituals whose window actually opened) kept deliberately separate from the page-grade `getRituals` so the per-minute tick costs ~2 Redis commands rather than ~17 per ritual — which returns the active rituals whose window opened within the last 5 minutes and are still un-acted; for each it fires `sendNotification(owner, ..., { bypassPresence: true })` and dedups via `ritual:fcm:sent:{ritualId}:{owningDateKey}` (`SET NX EX 36h`). Auth: `Authorization: Bearer ${CRON_SECRET}` — endpoint refuses to run if the env var is unset. **Single trigger: cron-job.org**, configured out-of-band in the operator's account, hits the URL every minute. There is intentionally no `vercel.json` in the repo — Vercel Hobby caps cron at once-per-day and rejects the build for any more-frequent schedule. Local notifications still fire on-device in parallel — different channels, no dedup needed.
...
- **Reactions are now feature-agnostic.** `src/app/actions/reactions.ts` exports `reactToTarget(type, id, emoji)` + `getReactionsForTargets(type, ids[])` keyed on a `ReactionTargetType` union (currently `"note" | "tod"`). The original `reactToNote` / `getReactionsForNotes` are thin wrappers that delegate to the generic API. **Storage asymmetry**: notes preserve the legacy unprefixed key shape `reactions:{id}` to avoid a one-time migration; new target types use the prefixed form `reactions:{type}:{id}`. The `reactionKeyFor(type, id)` helper inside `reactions.ts` is the single source of truth for this mapping — touch it once if a future migration ever consolidates them. UI: `<ReactionStrip>` in `src/components/reactions/` is the generic component (takes `onToggle` action prop + `variant: "default" | "compact"`); `<NoteReactions>` (notes feed) and `<TodReactions>` (TOD completed history rows) are thin wrappers supplying the appropriate `reactToTarget(...)` call. Toggling is restraint-gated for Kitten through the underlying action. Reactions on TOD render only on `status === "completed"` rows — refused/safeworded/expired/withdrawn/cancelled have no content worth reacting to.
- **Truth or Dare game (`/games/truth-or-dare`)** — symmetric play layered on top of a game-launcher framework at `/games`. The launcher reads `src/lib/games/registry.ts` (a typed `GameDescriptor[]`) and renders one tile per registered game; adding a new game is a registry entry plus the corresponding `src/app/games/{slug}/`, `src/app/actions/games/{slug}.ts`, `src/lib/games/{slug}-constants.ts`, `src/app/admin/games/{slug}/page.tsx`, and a unique Redis key prefix. For TOD specifically: either author can issue, but each issuance carries BOTH a truth prompt AND a dare prompt in one submission — the recipient picks which to engage with. Two single-slot sentinels (`tod:active:T7SEN` and `tod:active:Besho` STRINGs, each with the live TTL as the sentinel) cap each author's outgoing direction at one active challenge. State machine: `pending` (waiting for pick) → `picked` (truth or dare chosen, response phase) → `completed` (submitted) OR `refused` (recipient declined — Kitten penalty) OR `safeworded` (hard-no on this prompt — no penalty either direction) OR `expired` (cron past TTL — `TOD_PENDING_TTL_SEC = 72h` pre-pick, `TOD_PICKED_TTL_SEC = 48h` post-pick; pending dropped from 7d to 72h to match the conversational half-life) OR `withdrawn` (issuer cancelled — no penalty) OR `cancelled` (Sir admin force-cancel — no penalty). Game is symmetric; obedience scoring is asymmetric — only `recipient === "Besho"` emits, since Sir has no obedience score. Emit map at the action site: `submitResponse` fires `tod_truth_answered` (+3 default) or `tod_dare_completed` (+6); `refuseChallenge` fires `tod_refused` (−6); cron `expireDueChallenges` fires `tod_expired` (−10). Safeword and withdrawn intentionally emit nothing — they're free outs by design. Sir's recipient-side responses still increment the stats HASH but no score moves. Per-author stats live at `tod:stats:{author}` (`issued` / `truthsAnswered` / `daresCompleted` / `refused` / `safeworded` / `expired` / `withdrawn`) and both authors see both authors' stats at the bottom of the game page — same transparency as `/review` and `/rewards`. **Restraint interaction departs from the directive analog**: `assertWriteAllowed` blocks Kitten from `issueChallenge` and `withdrawChallenge` (initiating writes) but is intentionally NOT called from `pickPrompt` / `submitResponse` / `refuseChallenge` / `safewordChallenge` (responsive — otherwise an in-flight challenge stalls until restraint lifts, with no clean recovery). Sir is never restrained. FCM payload uses `data.kind === "tod_challenge"` + `challengeId`; `<FCMProvider>` foreground listener branches on the kind and dispatches `TOD_ARRIVED_EVENT` (`"ourspace:tod-arrived"`). The `/games/truth-or-dare` page itself is the listener — on event it re-fetches its bundle, surfacing the new challenge immediately rather than waiting for the next pull-to-refresh. When the recipient is on a different page, the FCM branch also fires the standard PushToast so they see the arrival; when they're already on the TOD page, the dispatch alone reconciles without a duplicate toast. Soft-delete via `moveToTrash` with `feature: "tod_challenges"` — Sir-only, terminal-state-only (refuses while pending/picked). Admin surface lives at `/admin/games` (registry-driven landing) and `/admin/games/truth-or-dare` (Active / History / Stats tabs): force-cancel a single challenge (no penalty, leaves `cancellationReason` audit), mass-cancel both active slots, per-stat edit + reset, purge-all (refuses while any active). The four new event weights register into `OBEDIENCE_EVENT_TYPES` + `DEFAULT_OBEDIENCE_WEIGHTS` and surface in the existing `/admin/rewards` Weights tab automatically — no separate weights UI. Active TOD count surfaces on the `/admin` landing dashboard strip alongside pending perms / claims / cron freshness / errors. Cron expiration shares `/api/cron/timer-expire` with directive + punishment sweeps — no new cron endpoint. The cron walker `warnExpiringChallenges` runs on the same tick and fires a one-shot "expires in ~Nh" push to the recipient when `expiresAt - now < TOD_EXPIRE_WARN_THRESHOLD_SEC` (24h default); dedups via `tod:fcm:expire-warn:{id}` SET NX EX (TTL bounded by the challenge's remaining lifetime so the key auto-evicts on natural expiry). Fires for both `pending` (you have 24h left to pick) and `picked` (24h to answer) states. **Component layout**: user page (`src/app/games/truth-or-dare/page.tsx`) owns bundle state + refresh subscriptions; the sections (`IncomingCard` + inline `PromptCard`, `OutgoingCard`, `IssueForm`, `HistoryRow`, `StatsStrip` + inline `StatsColumn`, `PageHeader`) live under `src/components/games/truth-or-dare/`. Admin page (`src/app/admin/games/truth-or-dare/page.tsx`) owns tab + bundle state; the tab bodies (`ActiveTab` + inline `MassCancelButton`/`ActiveRecordCard`, `HistoryTab` + inline `AdminHistoryRow`, `StatsTab` + inline `StatsEditor` + inline `WindowedColumn`, `PageHeader`, `TabButton`) live under `src/components/admin/games/truth-or-dare/`. Shared display utilities (`STATUS_CHIP`, `formatRelative`, `formatCountdown`) live in `truth-or-dare-constants.ts` + `truth-or-dare-utils.ts` so user + admin pages stay in sync. **v2 additions:** (1) **Reactions** on completed history rows via the generalized `<TodReactions>` wrapper — stored at `reactions:tod:{challengeId}` HASH (not preserved on soft-delete restore — auxiliary state). (2) **Streak counters** `currentStreak` + `longestStreak` are part of `TodStats` (same HASH at `tod:stats:{author}`). Recipient-side: increments on `submitResponse`, resets on `refuseChallenge` + cron-driven `tod_expired`; safeword and withdrawn do NOT reset (free outs). High-water mark tracked in `longestStreak`. (3) **Change-of-mind window** (`CHANGE_PICK_WINDOW_SEC = 60`) — `pickPrompt` accepts a `picked → picked` flip to the other type within 60s of `pickedAt`; the issuer notification is suppressed on the flip to avoid double-pinging. The UI surfaces a "Pick {other} instead · Ns" affordance only while the window is open. (4) **Prompt library** at `tod:prompts:{author}` JSON array (`TodPrompt[]` capped at `MAX_PROMPTS_PER_LIBRARY = 50`) — each author manages their own private library (no cross-author view, mirrors the `permissions:auto-rules` privacy pattern but per-author rather than Sir-only). Actions: `getTodPromptLibrary`, `saveTodPrompt`, `deleteTodPrompt`. UI: `<IssueForm>` exposes a Sheet-based picker + "Save to library" button. (5) **Pre-warning push** described above. (6) **Windowed admin stats** — `getTodWindowedStats(windowDays?)` aggregates the index ZSET by `createdAt` score range (or all-time when omitted) into per-author `TodWindowedCounters`. The Stats tab on `/admin/games/truth-or-dare` renders an All/30d/7d toggle above the existing editable cumulative counters. **v3 additions:** (a) **FCM excerpt** — `submitResponse` embeds the first ~80 chars of the response in the issuer-side FCM body via `formatResponseExcerpt` (word-boundary truncation with ellipsis); safe to expose on the lock screen because the device biometric gate is the access boundary. `RESPONSE_EXCERPT_LEN` is the tuning constant. (b) **Admin history search** — the History tab's filter input runs a pure client-side substring match across `truthPrompt` / `darePrompt` / `response` / `refuseReason` / `withdrawReason` / `adminCancelReason`; clearing the input restores the full list without a refetch. (c) **CLI surface** — `/api/admin/cli/tod` (`GET` status, `POST` issue, with single-slot guard mirroring `issueChallenge`) and `/api/admin/cli/tod/cancel` (force-cancel — defaults to whichever slot is active when no id is passed, preferring Sir's outgoing). Driven by `packages/cli/src/commands/tod.ts` exposing `ourspace tod status|issue|cancel`. CLI issuance is always Sir-to-Kitten by design — the CLI is Sir's terminal-side tool. (d) **`adminCancelReason` field** — the previously-named `cancellationReason` field on `TodChallenge` was renamed to `adminCancelReason` so the origin (Sir admin force-cancel) is clear next to `refuseReason` (recipient) and `withdrawReason` (issuer own-cancel).
...
└── cli/                            # `ourspace` desktop CLI — Sir-only, bearer-authed via ADMIN_CLI_TOKEN
    ...
    └── src/
        ...
        └── commands/
            ...
            ├── directive.ts        # issue / cancel / list real-time directives
            ├── punish.ts           # issue / cancel / list punishment timers
            ├── rules.ts            # list rules with UUIDs
            ├── violation.ts        # log rule-violation ledger entry
            └── tod.ts              # status | issue "<truth>" "<dare>" | cancel [<id>] [--reason "…"]
...
│   │   │   └── games.ts        # TOD admin bundle, force-cancel, mass-cancel, stat reset/edit, purge-all, active count
│   │   ├── rewards.ts          # Both-author bundle, claim/deliver/deny, claim history
│   │   ├── games/
│   │   │   └── truth-or-dare.ts # User-facing TOD actions (issue/pick/submit/refuse/safeword/withdraw/queries) + cron-only expireDueChallenges
...
│       │   ├── timer-expire/                # 5-minute cadence — sweeps directive timers (emits directive_missed), punishment timers (bails past endsAt + grace, emits punishment_bailed, auto-creates ledger entry), AND Truth or Dare challenges (expires past expiresAt, emits tod_expired for Kitten direction only)
...
│           └── tod/                         # POST {truthPrompt, darePrompt} — Sir-to-Kitten TOD challenge (single-slot guard); GET ?limit — both active slots + recent
│               └── cancel/                  # POST {id?, reason?} — force-cancel an active TOD challenge (no obedience penalty); id omitted → resolves to whichever slot is active
├── components/
│   ├── biometric-gate.tsx
│   ├── fcm-provider.tsx
...
│   └── games/                  # Per-game registry + constants + utils. registry.ts (GameDescriptor[] feeding launcher), truth-or-dare-constants.ts (status union, TTLs [72h pending / 48h picked], prompt/response bounds, Redis key helpers, stats shape, STATUS_CHIP color map, TOD_ARRIVED_EVENT name), truth-or-dare-utils.ts (formatRelative + formatCountdown pure helpers shared by user + admin pages)
...
| `/games` launcher + Truth or Dare — schema, state, single-slot | `references/redis-schema.md` § "Truth or Dare" |
