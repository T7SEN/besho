...
- **`will-change-transform` on elements that animate transforms repeatedly.** Promotes them to their own composited layer so transform animations don't repaint parents. Currently applied to: the page-transition `motion.div` in `src/app/template.tsx`, both `layoutId="navbar-active-indicator"` pills in the floating navbar, the CounterCard hero + anniversary AnimatePresence spans, the More-sheet drawer in the floating navbar, and the gift-box + reward-card + reward-emoji elements in `RewardUnlockOverlay` (the in-game-style claim reveal). Don't sprinkle this on every motion element — overuse explodes GPU layer count and degrades perf. Rule of thumb: add it where the same element transforms ≥ once per second OR on every navigation; otherwise leave it off.
