...
- **Three crons total.** `/api/cron/ritual-windows` (minute), `/api/cron/obedience-sweep` (daily), `/api/cron/review-window-open` (daily, internally gated to Saturday). All bearer-authed via `CRON_SECRET`. All triggered from cron-job.org.
...
| Standalone routes `/admin/inspector`, `/admin/cooldowns`, `/admin/time`, `/admin/timezone`                                                    | `/admin/devices` ("Right now" section) for inspector; `/admin/health` (Cooldowns + Time tabs — Time tab hosts BOTH the live snapshot and the Cairo↔Tabuk converter) for the other three |
